...
/**
 * @description Convert a date string from the format without "/" to a valid date DDMMYYYY with missing values => new Date(YYYY-MM-DD)
 * @param dateString DDMMYYYY
 * @returns new Date(YYYY-MM-DD)
 */
function rawDateToValidDate(dateString: string) {
  const now = new Date();
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === "Backspace") {
      return setStringDate((prev) => {
        ...
        setSelectedDate(rawDateToValidDate(sliced));
    ...
    if (isKeyDigit) {
      ...
      setStringDate((prev) => {
        ...
        setSelectedDate(rawDateToValidDate(newDate));
        return newDate;
      });
    }
  };
